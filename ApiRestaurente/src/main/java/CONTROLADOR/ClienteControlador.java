...
import MODELO.Usuarios; // Importa la clase 'Cliente' para el modelo de datos.
...
//    }
    ...
    @GET // Anotación para peticiones HTTP GET.
    @Path("verificar/{cedula}") // Mapea a la ruta '/clientes/{correo}'.
    public Response verificarSiExisteCliente(@PathParam("cedula") String cedula) { // Método para obtener un cliente por su correo.
        try { // Bloque try-catch.
            String validaCedula = Middlewares.validarEntero(cedula, "cedula"); // Valida el formato del correo.
            if (!validaCedula.equals("ok")) { // Si la validación falla...
                return Response.status(Response.Status.BAD_REQUEST).entity(validaCedula).build(); // ...retorna un 400 (Bad Request).
            }

            boolean existe = clienteDAO.obtenerPorCedulaBoolean(cedula);
            if (existe) { // Si la creación es exitosa...
                String mensaje = "Cliente cc." + cedula + " si Existe.";
  ...
  public Response crearCliente(Usuarios usuarios) {
        try {
            ...
            boolean creado = clienteDAO.crear(usuarios);
            if (creado) {
                String mensaje = "Cliente "+usuarios.getNombre() + " " + usuarios.getApellido() + " creado con EXITO.";
                return Response.status(Response.Status.CREATED)
                        .entity("{\"Ok\":\""+mensaje+"\"}")
                        .build();
            } else {
